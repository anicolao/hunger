# Accessible release journey

The primary phone journey respects the chosen appearance, keyboard and motion preferences, and remains usable at layout extremes.

## A paired check-in completes in system dark appearance with visible keyboard focus

![A paired check-in completes in system dark appearance with visible keyboard focus](./screenshots/000-dark-phone-keyboard-journey-phone-darwin.png)

**Verifications:**

- [x] Chosen dark colors are active without changing semantic content
- [x] Reduced motion and keyboard focus remain observable

## Settings remains semantic at 200% text, forced colors, smallest phone, landscape, and tablet sizes

![Settings remains semantic at 200% text, forced colors, smallest phone, landscape, and tablet sizes](./screenshots/001-appearance-and-size-matrix-phone-darwin.png)

**Verifications:**

- [x] The iOS-style preferences retain accessible switch names
- [x] The page has no horizontal overflow in the canonical phone viewport
